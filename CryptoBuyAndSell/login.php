...
// header('location:CryptoBuyAndSellHome.php');
// var_dump($authenticationResponse);
	// print_r($authenticationResponse);
	// print_r($authenticationResponse[3]);

	// print_r(array_values($authenticationResponse));
	
/** @var \pmill\AwsCognito\CognitoClient $client */
$client = require(__DIR__ . '/bootstrap.php');

// $accessToken = $authenticationResponse;
$accessToken=(array_values($authenticationResponse)[0]);
// print_r(array_keys($authenticationResponse)[3]);
...
$username = $username;
$refreshToken = (array_values($authenticationResponse)[3]);
...
$y=$refreshToken;
 ...
 $_SESSION['sessionRefresh'] = $y;
header('location:CryptoBuyAndSellHome.php');
// header('location:verifyToken.php?u='.$username);

	// $text = $authenticationResponse;
	// echo $text;
// $art=explode('"',(string)$text[0]);
// echo $art[0];
// for ($inc = 0; $inc < 4; $inc++) {
  // echo "<p><b>Row number $inc</b></p>";
  // echo "<ul>";
    // echo "<li>".$art[$inc]."</li>";
  // echo "</ul>";
// }
					...
					<form class="s12" action="" method="post">
						...
						<div>
							<div class="input-field s4">
								<input type="submit" name="submit" value="Login" class="waves-effect waves-light log-in-btn"> </div>
						</div>
						<div>
